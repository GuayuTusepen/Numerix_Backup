...
const ClassifyAndCountGame: React.FC<Props> = ({ gameLevel, onLevelComplete }) => {
  ...
  if (gameState === "welcome") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-200 via-pink-200 to-purple-300 flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl mx-auto shadow-2xl border-4 border-rainbow">
  ...
  if (gameState === "completed") {
    const stars = Math.max(1, Math.min(3, Math.floor((gameLevel.objects.length / attempts) * 3)))
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => setGameState("welcome")} variant="outline" size="sm">
                  🏠 Inicio
                </Button>
              </div>
            </div>
